...
def general_graph(df, x, y, bars, multidimname, multidimval):
    ##x -> size
    ##y -> time (accuracy ?)
    ##multidim -> density
    ...
    for val in multidimval:
        dfs.append(df.loc[(df[multidimname] == val) & (df['decomposition'] == 0)])
        dfs.append(df.loc[(df[multidimname] == val) & (df['decomposition'] == 1)])
...
if __name__ == "__main__":
    ...
    algos = ['gpu_perman_xglobal',
             ...
             'gpu_perman_xshared_coalescing_mshared_sparse',
             'gpu_perman_xshared_coalescing_mshared_skipper',
             'gpu_perman_xshared_coalescing_mshared_multigpu_sparse']
